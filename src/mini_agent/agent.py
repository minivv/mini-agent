"""
Phase 7-mw: LangGraph ReAct Agent — create_agent + SandboxMiddleware + SummarizationMiddleware + SubagentLimitMiddleware

旧方式 (Phase 7 原版):
  手动 StateGraph + AgentState (含 skills_dir)
  subagent_limit 节点截断多余的 task 调用
...
class LangGraphAgent:
    """用 create_agent + SandboxMiddleware + SummarizationMiddleware + SubagentLimitMiddleware 构建的 Agent。"""
    ...
    def __init__(
        ...
        self.graph = create_agent(
            model=self.model,
            tools=self.tools,
            system_prompt=build_system_prompt(skills_section, subagent_section),
            middleware=mw_list,
