...
2. State — 在节点间流动的数据
   ...
   DeerFlow 的 ThreadState 就比我们多了 sandbox, artifacts, todos 等字段。
...
class LangGraphAgent:
    ...
    def __init__(self, config: Config, tools: list):
        ...
        self.checkpointer = MemorySaver()  # 内存持久化, 按 thread_id 隔离
    ...
    def _build_graph(self):
        ...
        # 条件边: tools_condition 读取最后一条消息
        #   如果有 tool_calls → 去 tools 节点
        #   如果没有         → 去 END
        builder.add_conditional_edges("agent", tools_condition)

        # 普通边: tools 执行完后总是回到 agent（让 LLM 看到工具结果后继续思考）
    ...
    def stream(self, user_input: str, thread_id: str = "default", debug: bool = False):
        ...
        # Phase 3: 为当前线程创建沙箱，注入 workspace_dir
