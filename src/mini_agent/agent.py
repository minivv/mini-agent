"""
Phase 6-mw: LangGraph ReAct Agent — create_agent + SandboxMiddleware + SummarizationMiddleware + Skills

旧方式 (Phase 6 原版):
  手动 StateGraph + AgentState (含 skills_dir)
  Sandbox(thread_id=..., skills_dir=...) 注入 workspace_dir 和 skills_dir
...
class LangGraphAgent:
    ...
    def __init__(
        ...
        self.graph = create_agent(
            model=self.model,
            tools=self.tools,
            system_prompt=build_system_prompt(skills_section),
            middleware=[
