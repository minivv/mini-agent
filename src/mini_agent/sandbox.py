...
class Sandbox:
    ...
    def __init__(
        ...
        skills_dir: Skills 目录路径（Phase 6），映射到 /mnt/skills/
        """
        if workspace_dir:
        ...
        # 构建虚拟挂载点映射（虚拟路径 → 真实路径）
        self.VIRTUAL_MOUNTS = {"/mnt/workspace": self.workspace}

        # Phase 6: Skills 目录 → /mnt/skills/
        self.skills_dir = Path(skills_dir).resolve() if skills_dir else None
        if self.skills_dir and self.skills_dir.is_dir():
            self.VIRTUAL_MOUNTS["/mnt/skills"] = self.skills_dir
    ...
    def _resolve(self, virtual_path: str, must_exist: bool = False) -> Path:
        ...
        resolved = None
        mount_root = None
        for mount, real_path in self.VIRTUAL_MOUNTS.items():
            if virtual_path == mount:
                resolved = real_path
                mount_root = real_path
                break
            if virtual_path.startswith(mount + "/"):
                relative = virtual_path[len(mount) + 1:]
                resolved = Path(real_path) / relative
                mount_root = real_path
                break
        ...
        if resolved is None:
            ...
            mount_root = self.workspace

        # 规范化并检查路径穿越
        resolved = resolved.resolve()
        if not str(resolved).startswith(str(Path(mount_root).resolve())):
    ...
    def _translate_command(self, command: str) -> str:
        ...
        for mount, real_path in self.VIRTUAL_MOUNTS.items():
            ...
            # /mnt/skills/bar.py    → {skills_dir}/bar.py
            translated = translated.replace(mount + "/", str(real_path) + "/")
            translated = translated.replace(mount, str(real_path))
        return translated
