...
def create_model(config: Config, streaming: bool = True) -> ChatOpenAI:
    ...
    自动检测 DeepSeek 并切换到 PatchedChatDeepSeek，
    解决 thinking 模式下 reasoning_content 丢失的问题。
    """
    # 构建额外参数
    extra_body: dict = {}
    ...
    # DeepSeek API: 使用 PatchedChatDeepSeek 保留 reasoning_content
    if "deepseek" in config.base_url.lower():
        from mini_agent.models import PatchedChatDeepSeek
...
你有一个沙箱环境，可以读写文件和执行命令:
- 文件路径使用 /mnt/workspace/ 前缀 (如 /mnt/workspace/main.py)
- 可以使用 write_file 创建文件，read_file 读取文件，ls 查看目录
- 可以使用 bash 执行命令 (python, node, git 等)
- 所有操作都隔离在沙箱内，不会影响宿主系统"""
...
def build_subagent_section(max_concurrent: int = 3) -> str:
    """构建子 Agent 系统提示词段。

    告诉 LLM 如何分解任务、委派子 Agent、综合结果。
    """
    from mini_agent.subagents.registry import get_subagent_names

    available_names = get_subagent_names()

    # 构建子 Agent 描述列表
    agent_descriptions = []
    for name in available_names:
        from mini_agent.subagents.registry import get_subagent_config
        config = get_subagent_config(name)
        if config:
            desc = config.description.replace("\n", " ").strip()
            agent_descriptions.append(f"- **{name}**: {desc}")

    agents_block = "\n".join(agent_descriptions) if agent_descriptions else "- (no subagents available)"
...
- Each response, you may include **at most {n}** `task` tool calls. Any excess calls are **silently discarded**.
- If you have more sub-tasks than {n}, launch the most important ones first, save the rest for the next turn.
...
- Multi-aspect analysis requiring different perspectives
...
class ChatAgent:
    ...
    这就是最基本的 "Agent 循环"。后续 Phase 会在此基础上添加:
...
# ---------------------------------------------------------------------------
# Phase 8: Todo List 系统提示词
# ---------------------------------------------------------------------------

def build_todo_section() -> str:
    """构建 Todo List 系统提示词段。"""
    return """
<todo_system>
**TODO LIST MODE ACTIVE**

You have access to a `write_todos` tool for planning and tracking multi-step tasks.

**Usage guidelines:**
- For complex tasks (3+ steps), create a todo list BEFORE starting work
- Call `write_todos` with the COMPLETE list every time you update status
- Only ONE item should be "in_progress" at a time
- Transition: "pending" → "in_progress" → "completed"
- Update the list promptly as you complete each step

**Example workflow:**
1. User asks: "Build a web scraper"
2. You call write_todos with: [design, implement, test]
3. Start working on "design", update to "in_progress"
4. Finish "design", update to "completed", move "implement" to "in_progress"
5. Continue until all items are "completed"
</todo_system>
"""
