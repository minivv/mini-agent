"""
Phase 6-mw: CLI 入口 — create_agent + Skills + MCP + SandboxMiddleware + SummarizationMiddleware
...
def print_banner() -> None:
    ...
    print()
    print("╔══════════════════════════════════════════╗")
    print("║        Mini Agent - Phase 6              ║")
    print("║  Skills: 可复用的知识模块                  ║")
    print("╠══════════════════════════════════════════╣")
    print("║  命令:                                   ║")
...
def handle_command(cmd: str, agent: LangGraphAgent, thread_id: list, mcp_tools: list = None, skills: list = None) -> bool:
    """处理斜杠命令。"""
...
def main() -> None:
    ...
    # 4. 创建 LangGraph Agent (绑定全部工具: 内置 + 沙箱 + MCP)
    ...
    while True:
        ...
        if user_input.startswith("/"):
            if not handle_command(user_input, agent, thread_id, mcp_tools, skills):
                break
            print()
            continue
